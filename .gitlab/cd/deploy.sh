...
if [ -n "${APP_IMAGE_REGISTRY_USERNAME:-}" ] && [ -n "${APP_IMAGE_REGISTRY_PASSWORD:-${BASE_IMAGE_REGISTRY_PASSWORD:-}}" ]; then
  APP_IMAGE_REGISTRY_PASSWORD_B64="$(printf '%s' "${APP_IMAGE_REGISTRY_PASSWORD:-${BASE_IMAGE_REGISTRY_PASSWORD:-}}" | base64 | tr -d '\n')"
fi
